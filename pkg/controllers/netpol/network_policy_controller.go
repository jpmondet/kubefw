...
const (
	...
	kubePodFirewallChainPrefix   = "KFW-POD-FW-"
	kubeNetworkPolicyChainPrefix = "KFW-NWPLCY-"
	kubeSourceIPSetPrefix        = "KFW-SRC-"
	kubeDestinationIPSetPrefix   = "KFW-DST-"
)
...
type NetworkPolicyController struct {
	...
	// Used to get all the rules on standalone mode
	standalone bool
...
func (npc *NetworkPolicyController) syncNetworkPolicyChains(version string) (map[string]bool, map[string]bool, error) {
	...
	for _, policy := range *npc.networkPoliciesInfo {
		...
		if err != nil {
			glog.Errorf("failed to refresh targetDestPodIPSetName: " + err.Error())
		}
		...
		err = npc.processEgressRules(policy, targetDestPodIPSetName, activePolicyIPSets, version)
		if err != nil {
			return nil, nil, err
		}
	}
...
//ListPodsByNamespaceAndLabels Outputs list of Pod API objects
...
//ListNamespaceByLabels Outputs list of namespace API objects
...
func (npc *NetworkPolicyController) Cleanup() {
	...
	glog.Info("Cleaning up iptables configuration permanently done by kubefw")
	...
	glog.Infof("Successfully cleaned the iptables configuration done by kubefw")
}
...
func NewNetworkPolicyController(clientset kubernetes.Interface,
	config *options.KubeFwConfig, podInformer cache.SharedIndexInformer,
